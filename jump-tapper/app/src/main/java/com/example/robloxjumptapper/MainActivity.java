...
public class MainActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        TextView title = new TextView(this); title.setText("Jump Tapper v1.20"); title.setTextSize(28); title.setTypeface(Typeface.DEFAULT_BOLD); root.addView(title);
        ...
        addLabeledField(splitFieldsRow,hoursField,"h (0–1000)"); addLabeledField(splitFieldsRow,minutesField,"m (0–1000)"); addLabeledField(splitFieldsRow,secondsField,"s (0–1000)"); addLabeledField(splitFieldsRow,millisField,"ms (0–1000)"); root.addView(splitFieldsRow);
        ...
        tapDurationField = number1000(prefs.getInt("tap_duration_ms",30));
        startDelayField = number1000(prefs.getInt("start_delay_ms",0));
        addLabeledField(tapTuneRow,tapDurationField,"Tap hold ms 0–1000"); addLabeledField(tapTuneRow,startDelayField,"Start delay ms 0–1000"); root.addView(tapTuneRow);
        ...
        recordStartStopKey.setOnClickListener(v -> {
            ...
            startStopCustomKeyLabel.setText("Listening… hold one or more buttons, then release all");
            Toast.makeText(this, "Hold your Start/Stop button(s), then release all", Toast.LENGTH_LONG).show();
        });
        root.addView(recordStartStopKey);
        startStopCustomKeyLabel = new TextView(this);
        startStopCustomKeyLabel.setText(comboLabel(prefs,"startstop_hotkey_codes","startstop_hotkey_names","No Start/Stop button combo recorded yet"));
        ...
        startStopNote.setText("Recording supports one button or multiple buttons held together. The full combo must be down before Start/Stop triggers. If a combo overlaps the overlay combo, Start/Stop gets priority.");
        ...
        Button recordKey = new Button(this); recordKey.setText("Record overlay button(s)");
        recordKey.setOnClickListener(v -> {
            ...
            customKeyLabel.setText("Listening… hold one or more buttons, then release all");
            Toast.makeText(this, "Hold your overlay button(s), then release all", Toast.LENGTH_LONG).show();
        });
        root.addView(recordKey);
        customKeyLabel = new TextView(this);
        customKeyLabel.setText(comboLabel(prefs,"custom_hotkey_codes","custom_hotkey_names","No custom overlay button combo recorded yet"));
        ...
        hotkeyNote.setText("Custom recording supports multiple phone, USB/Bluetooth keyboard, or controller buttons held together. Android may reserve some buttons such as Power/Home.");
    ...
    private void migrateSettings(SharedPreferences p) {
        ...
        if(!p.getBoolean("numeric_schema_1000",false)){
            ...
            e.putBoolean("numeric_schema_1000",true);changed=true;
        }
        if(changed)e.apply();
    }

    private String friendlyCombo(String names,String codes){
        if(names==null)names=""; if(codes==null)codes="";
        String[] ns=names.split(","), cs=codes.split(","); StringBuilder b=new StringBuilder();
        int n=Math.max(ns.length,cs.length); for(int i=0;i<n;i++){String name=i<ns.length?ns[i]:"";String code=i<cs.length?cs[i]:"";if(name.startsWith("KEYCODE_"))name=name.substring(8);name=name.replace('_',' ');if(name.isEmpty())name="KEY "+code;if(b.length()>0)b.append(" + ");b.append(name);if(!code.isEmpty())b.append(" (").append(code).append(")");}
    ...
    private void saveSettings(){
        ...
        try{
            if(millisMode.isChecked()) interval=parseLong(totalMillisField); else {
                long h=parseLong(hoursField),m=parseLong(minutesField),s=parseLong(secondsField),ms=parseLong(millisField);
                if(h>1000||m>1000||s>1000||ms>1000){Toast.makeText(this,"H / M / S / ms boxes accept 0–1000.",Toast.LENGTH_LONG).show();return;}
        ...
        if(!in1000(targetSize)||!in1000(controlScale)||!in1000(tapDuration)||!in1000(startDelay)||!in1000(targetOpacity)||!in1000(controlOpacity)||!in1000(hapticStrength)){Toast.makeText(this,"Every numeric tuning box must be 0–1000.",Toast.LENGTH_LONG).show();return;}
        int hotkeyMode=hotkeySpinner.getSelectedItemPosition(), startStopMode=startStopHotkeySpinner.getSelectedItemPosition();
        ...
        current.edit()
                ...
                .putInt("target_opacity_1000",targetOpacity).putInt("control_opacity_1000",controlOpacity)
                ...
                .putBoolean("haptic_enabled",hapticEnabled.isChecked()).putInt("haptic_strength_1000",hapticStrength)
                .putBoolean("debug_enabled",debugEnabled.isChecked()).putInt("overlay_hotkey",hotkeyMode)
                .putInt("startstop_hotkey_mode",startStopMode).putBoolean("hotkey_schema_v2",true).putBoolean("numeric_schema_1000",true).apply();
    ...
    private void shareLog(){Intent s=new Intent(Intent.ACTION_SEND);s.setType("text/plain");s.putExtra(Intent.EXTRA_SUBJECT,"Jump Tapper v1.20 Debug Log");s.putExtra(Intent.EXTRA_TEXT,readLog());startActivity(Intent.createChooser(s,"Share debug log"));}
