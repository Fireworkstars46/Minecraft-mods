...
public class TapAccessibilityService extends AccessibilityService {
    ...
    private final BroadcastReceiver reloadReceiver = new BroadcastReceiver() {
        @Override public void onReceive(Context context, Intent intent) {
            loadSettings(); applyOverlaySettings();
    ...
    @Override protected void onServiceConnected() {
        ...
        loadSettings(); createTarget(); createControl(); createRestoreBubble(); applyOverlaySettings();
    ...
    private void createControl() {
        ...
        TextView dragHandle = new TextView(this); dragHandle.setText("≡"); dragHandle.setTextColor(Color.WHITE);
        dragHandle.setGravity(Gravity.CENTER); dragHandle.setBackgroundColor(0xFF3C4043);
        ...
        startStop.setOnClickListener(v -> toggleRunning()); control.addView(startStop, new LinearLayout.LayoutParams(scaled(84), scaled(46)));
        moveButton = new Button(this); moveButton.setText(moveMode ? "LOCK" : "MOVE");
        moveButton.setOnClickListener(v -> setMoveMode(!moveMode)); control.addView(moveButton, new LinearLayout.LayoutParams(scaled(66), scaled(46)));
        Button tapNow = new Button(this); tapNow.setText("TAP"); tapNow.setOnClickListener(v -> performTargetTap(true));
        control.addView(tapNow, new LinearLayout.LayoutParams(scaled(58), scaled(46)));
        Button targetToggle = new Button(this); targetToggle.setText("J"); targetToggle.setOnClickListener(v -> {
            targetVisible = !targetVisible; getSharedPreferences(MainActivity.PREFS, MODE_PRIVATE).edit().putBoolean("target_visible", targetVisible).apply();
        ...
        Button close = new Button(this); close.setText("×"); close.setOnClickListener(v -> hideControls());
    ...
    private void createRestoreBubble() {
        ...
        restoreBubble = new TextView(this); restoreBubble.setText("+"); restoreBubble.setTextColor(Color.WHITE); restoreBubble.setTextSize(22);
        restoreBubble.setGravity(Gravity.CENTER); restoreBubble.setTypeface(Typeface.DEFAULT_BOLD);
        restoreBubble.setBackground(circleBackground(0xDD3C4043, 0xFFFFFFFF));
        int size = dp(44);
        restoreParams = new WindowManager.LayoutParams(size, size, WindowManager.LayoutParams.TYPE_ACCESSIBILITY_OVERLAY,
                WindowManager.LayoutParams.FLAG_NOT_FOCUSABLE | WindowManager.LayoutParams.FLAG_LAYOUT_IN_SCREEN, PixelFormat.TRANSLUCENT);
        restoreParams.gravity = Gravity.TOP | Gravity.START;
        SharedPreferences prefs = getSharedPreferences(MainActivity.PREFS, MODE_PRIVATE);
        restoreParams.x = prefs.getInt("restore_x", dp(8)); restoreParams.y = prefs.getInt("restore_y", dp(180));
        restoreBubble.setOnTouchListener(new BubbleTouchListener()); windowManager.addView(restoreBubble, restoreParams);
    }
    ...
    private void applyOverlaySettings() {
        if (target != null && targetParams != null) {
            int size=dp(targetSizeDp); targetParams.width=size; targetParams.height=size; target.setTextSize(Math.max(12,targetSizeDp/3f));
            target.setVisibility(targetVisible?View.VISIBLE:View.GONE); applyTargetTouchability();
        }
        if(control!=null){try{windowManager.removeView(control);}catch(Exception ignored){} control=null;startStop=null;moveButton=null;createControl();control.setVisibility(controlVisible?View.VISIBLE:View.GONE);}
    ...
    private void setMoveMode(boolean enabled){moveMode=enabled;if(moveButton!=null)moveButton.setText(moveMode?"LOCK":"MOVE");applyTargetTouchability();}
    private void toggleRunning(){running=!running;tapInProgress=false;if(startStop!=null)startStop.setText(running?"STOP":"START");handler.removeCallbacks(tapLoop);applyTargetTouchability();if(running)handler.postDelayed(tapLoop,150L);}

    private void performTargetTap(boolean manual){
        if(target==null||targetParams==null||windowManager==null||tapInProgress)return;
        int[] location=new int[2];target.getLocationOnScreen(location);final float x=location[0]+target.getWidth()/2f;final float y=location[1]+target.getHeight()/2f;tapInProgress=true;
        targetParams.flags |= WindowManager.LayoutParams.FLAG_NOT_TOUCHABLE; try{windowManager.updateViewLayout(target,targetParams);}catch(Exception ignored){}
        Path path=new Path();path.moveTo(x,y);GestureDescription.StrokeDescription stroke=new GestureDescription.StrokeDescription(path,0L,10L);
        GestureDescription gesture=new GestureDescription.Builder().addStroke(stroke).build();
        boolean accepted=dispatchGesture(gesture,new GestureResultCallback(){@Override public void onCompleted(GestureDescription g){finishTap(manual);}@Override public void onCancelled(GestureDescription g){finishTap(manual);}},null);
        if(!accepted)finishTap(manual);
    }
    private void finishTap(boolean manual){tapInProgress=false;if(!running)applyTargetTouchability();}
    ...
    private class DragTouchListener implements View.OnTouchListener{
        private final View movedView;private final WindowManager.LayoutParams params;private final String xKey,yKey;private int startX,startY;private float downX,downY;
        ...
        @Override public boolean onTouch(View v,MotionEvent event){switch(event.getActionMasked()){case MotionEvent.ACTION_DOWN:startX=params.x;startY=params.y;downX=event.getRawX();downY=event.getRawY();return true;case MotionEvent.ACTION_MOVE:params.x=startX+Math.round(event.getRawX()-downX);params.y=startY+Math.round(event.getRawY()-downY);try{windowManager.updateViewLayout(movedView,params);}catch(Exception ignored){}return true;case MotionEvent.ACTION_UP:case MotionEvent.ACTION_CANCEL:getSharedPreferences(MainActivity.PREFS,MODE_PRIVATE).edit().putInt(xKey,params.x).putInt(yKey,params.y).apply();return true;}return true;}
    }
    private class BubbleTouchListener implements View.OnTouchListener{
        private int startX,startY;private float downX,downY;private boolean moved;
        @Override public boolean onTouch(View v,MotionEvent event){switch(event.getActionMasked()){case MotionEvent.ACTION_DOWN:startX=restoreParams.x;startY=restoreParams.y;downX=event.getRawX();downY=event.getRawY();moved=false;return true;case MotionEvent.ACTION_MOVE:float dx=event.getRawX()-downX,dy=event.getRawY()-downY;if(Math.abs(dx)>dp(4)||Math.abs(dy)>dp(4))moved=true;restoreParams.x=startX+Math.round(dx);restoreParams.y=startY+Math.round(dy);try{windowManager.updateViewLayout(restoreBubble,restoreParams);}catch(Exception ignored){}return true;case MotionEvent.ACTION_UP:getSharedPreferences(MainActivity.PREFS,MODE_PRIVATE).edit().putInt("restore_x",restoreParams.x).putInt("restore_y",restoreParams.y).apply();if(!moved)showControls();return true;case MotionEvent.ACTION_CANCEL:return true;}return true;}
    }
}
