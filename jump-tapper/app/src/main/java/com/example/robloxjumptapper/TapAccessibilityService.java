...
public class TapAccessibilityService extends AccessibilityService {
    ...
    private final BroadcastReceiver reload=new BroadcastReceiver(){@Override public void onReceive(Context c,Intent i){
        ...
        load();log("SETTINGS_RELOAD intervalMs="+intervalMs+" debug="+debug+" hotkeyMode="+hotkeyMode);apply();
    }};
    ...
    @Override protected void onServiceConnected(){
        ...
        load();
        log("SERVICE_CONNECTED sdk="+Build.VERSION.SDK_INT+" hotkeyMode="+hotkeyMode);
        createTarget();createControl();apply();
        IntentFilter f=new IntentFilter(MainActivity.ACTION_RELOAD);
        if(Build.VERSION.SDK_INT>=33)registerReceiver(reload,f,Context.RECEIVER_NOT_EXPORTED);else registerReceiver(reload,f);
    }
    ...
    private void load(){
        ...
        if(hotkeyMode<0||hotkeyMode>3)hotkeyMode=0;
    }
    ...
    private void createTarget(){
        ...
        target=new TextView(this);target.setText("+");target.setTextColor(Color.WHITE);target.setTypeface(Typeface.DEFAULT_BOLD);target.setGravity(Gravity.CENTER);target.setBackground(circle(0x161976D2,0xCC42A5F5));target.setAlpha(.9f);
    ...
    private void createControl(){
        ...
        control=new LinearLayout(this);control.setOrientation(LinearLayout.HORIZONTAL);control.setGravity(Gravity.CENTER_VERTICAL);control.setBackgroundColor(0xDD202124);
        ...
        tapButton=new Button(this);tapButton.setText("TAP");tapButton.setOnClickListener(v->performTargetTap(true));buttons.addView(tapButton,new LinearLayout.LayoutParams(scaled(58),scaled(46)));
        targetButton=new Button(this);targetButton.setText("J");targetButton.setOnClickListener(v->{if(running)return;targetVisible=!targetVisible;getSharedPreferences(MainActivity.PREFS,MODE_PRIVATE).edit().putBoolean("target_visible",targetVisible).apply();if(target!=null)target.setVisibility(targetVisible?View.VISIBLE:View.GONE);log("TARGET_VISIBLE="+targetVisible);});buttons.addView(targetButton,new LinearLayout.LayoutParams(scaled(46),scaled(46)));
    ...
    private void toggleOverlayHotkey(){
        ...
        log("HOTKEY_OVERLAY_HIDDEN="+hotkeyHidden+" mode="+hotkeyMode);
    }

    private void compensateFirstComboVolumeKey(){
        if(comboFirstKey==KeyEvent.KEYCODE_VOLUME_UP){
            AudioManager a=(AudioManager)getSystemService(AUDIO_SERVICE);
            if(a!=null)a.adjustStreamVolume(AudioManager.STREAM_MUSIC,AudioManager.ADJUST_LOWER,0);
        }else if(comboFirstKey==KeyEvent.KEYCODE_VOLUME_DOWN){
            AudioManager a=(AudioManager)getSystemService(AUDIO_SERVICE);
            if(a!=null)a.adjustStreamVolume(AudioManager.STREAM_MUSIC,AudioManager.ADJUST_RAISE,0);
        }
    }
        ...
        if(hotkeyMode==0 && (code==KeyEvent.KEYCODE_VOLUME_UP||code==KeyEvent.KEYCODE_VOLUME_DOWN)){
            ...
            if(action==KeyEvent.ACTION_DOWN && event.getRepeatCount()==0){
                if(code==KeyEvent.KEYCODE_VOLUME_UP){
                    volUpDown=true;lastVolUpDown=now;
                    if(!volDownDown)comboFirstKey=KeyEvent.KEYCODE_VOLUME_UP;
                }else{
                    volDownDown=true;lastVolDownDown=now;
                    if(!volUpDown)comboFirstKey=KeyEvent.KEYCODE_VOLUME_DOWN;
                }
                boolean together=volUpDown&&volDownDown&&Math.abs(lastVolUpDown-lastVolDownDown)<=350L;
                if(together&&!comboLatched){
                    comboLatched=true;
                    compensateFirstComboVolumeKey();
                    toggleOverlayHotkey();
                    return true;
                }
